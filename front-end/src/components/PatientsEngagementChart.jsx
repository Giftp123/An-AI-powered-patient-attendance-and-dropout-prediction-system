...
const PatientsEngagementChart = ({ patients }) => {
    const patientCounts = patients.reduce((acc, patient) => {
    const status = patient.engagement_status;
  ...
  const data = Object.keys(patientCounts).map((key) => ({
    name: key,
    value: patientCounts[key],
  }));
...
export default PatientsEngagementChart;
